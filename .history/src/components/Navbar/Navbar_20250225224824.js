...
const Navbar = () => {
  ...
  return (
    <nav
      className={`fixed top-0 left-0 w-full h-[64px] flex items-center justify-center z-[999] transition-transform duration-500 ${
        isVisible ? "translate-y-0" : "-translate-y-full"
      } ${isAtTop ? "bg-transparent" : "bg-background-dark/90 shadow-md"}
      `}
    >
      <div className="w-full flex items-center justify-between py-4 px-6 lg:px-12 box-border">
        {/* Left links */}
        <ul className="hidden max-[900px]:hidden flex items-center gap-8">
          <li>
