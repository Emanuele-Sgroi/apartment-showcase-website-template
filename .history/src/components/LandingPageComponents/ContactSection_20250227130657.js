...
const ContactSection = () => {
  ...
  const handleSubmit = (e) => {
    ...
    // Email validation (only if email isn't empty)
    if (formData.email && !isValidEmail(formData.email) && !generalError) {
      newErrors.email = "Please enter a valid email address.";
    }

    // Phone number validation (only if number isn't empty)
    if (formData.phone && !isValidPhone(formData.phone) && !generalError) {
  ...
  return (
    ...
    >
      ...
      {errors.email && !generalError && !errors.phone && (
        <p className="text-red-500 text-sm mt-4 text-center">{errors.email}</p>
      )}
      {errors.phone && !generalError && !errors.email && (
        <p className="text-red-500 text-sm mt-4 text-center">{errors.phone}</p>
      )}
    </section>
  );
};
