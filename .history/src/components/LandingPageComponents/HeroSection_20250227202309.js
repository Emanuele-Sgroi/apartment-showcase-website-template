...
const HeroSection = ({ homepageContent }) => {
  ...
  const { heroTitle, heroParagraph, heroDisplayMode } = homepageContent;
  ...
  return (
    <section className="relative w-full min-h-screen h-full flex justify-center items-center p-6">
      ...
      <div className="absolute top-0 left-0 w-full h-full z-[-2]">
        {heroDisplayMode === "carousel" && <ImageSlider />}
        {heroDisplayMode === "video" && <BackgroundVideo />}
        {heroDisplayMode === "image" && <SingleImageBg />}
      </div>
    </section>
  );
};
